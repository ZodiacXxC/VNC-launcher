...
def start_thread():
    ...
    if selected_item:
        ...
        port = 5900
        password = "firasawni"
        new_thread = Thread(target=select_suggestion, args=(host, port, password))
        new_thread.start()
...
def select_suggestion(host, port, password):
    try:
        command = ["C:\\Program Files\\RealVNC\\VNC Viewer\\vncviewer.exe", host + "::" + str(port), "-passwd", password]
        print("Executing command:", " ".join(command))
        subprocess.run(command)
...
cols = ("ID", "Namne", "IP")
treeview = ttk.Treeview(treeFrame, show="headings", yscrollcommand=treeScroll.set, columns=cols, height=22)
treeview.column("ID", width=50)
treeview.column("Namne", width=180)
treeview.column("IP", width=80)
treeview.bind("<Double-Button-1>", lambda e: Thread(target=start_thread).start())
treeview.pack()
